...
export class TimeSelectService {
  ...
  shouldShowDecrease(config: ITimeSelectConfigInternal, time: Dayjs, unit: TimeUnit): boolean {
    ...
    return (!config.min || (config.min.isSame(newTime) || config.min.isBefore(newTime)))
      && (!config.minTime || (config.minTime.isSame(this.utilsService.onlyTime(newTime)) || config.minTime.isAfter(this.utilsService.onlyTime(newTime))));
  }
  ...
  shouldShowIncrease(config: ITimeSelectConfigInternal, time: Dayjs, unit: TimeUnit): boolean {
    ...
    return (!config.max || (config.max.isSame(newTime) || config.max.isAfter(newTime)))
      && (!config.maxTime || (config.maxTime.isSame(this.utilsService.onlyTime(newTime)) || config.maxTime.isAfter(this.utilsService.onlyTime(newTime))));
  }
  ...
  shouldShowToggleMeridiem(config: ITimeSelectConfigInternal, time: Dayjs): boolean {
    ...
    return (!config.max || (config.max.isSame(newTime) || config.max.isAfter(newTime)))
      && (!config.min || (config.min.isSame(newTime) || config.max.isBefore(newTime)))
      && (!config.maxTime || (config.maxTime.isSame(this.utilsService.onlyTime(newTime)) || config.maxTime.isAfter(this.utilsService.onlyTime(newTime))))
      && (!config.minTime || (config.minTime.isSame(this.utilsService.onlyTime(newTime)) || config.minTime.isBefore(this.utilsService.onlyTime(newTime))));
  }
}
