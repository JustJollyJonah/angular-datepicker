...
export class UtilsService {
  ...
  updateSelected(isMultiple: boolean,
                 ...
                 date: IDate,
                 granularity: dayjs.OpUnitType = 'day'): Dayjs[] {
    if (isMultiple) {
  ...
  createValidator({minDate, maxDate, minTime, maxTime}: DateLimits,
    ...
    if (minTime) {
      ...
      validators.push({
        key: 'minTime',
        isValid: () => {
          const _isValid = value.every(val => (this.onlyTime(val).isSame(md) || this.onlyTime(val).isAfter(md)));
          isValid = isValid ? _isValid : false;
          return _isValid;
        }
      });
    ...
    if (maxTime) {
      ...
      validators.push({
        key: 'maxTime',
        isValid: () => {
          const _isValid = value.every(val => (this.onlyTime(val).isSame(md) || this.onlyTime(val).isBefore(md)));
          isValid = isValid ? _isValid : false;
          return _isValid;
        }
      });
  ...
  isDateInRange(date: Dayjs, from: Dayjs, to: Dayjs): boolean {
    return this.isBetween(date, from, to, 'day', '[]');
  }

  isBetween(needle: Dayjs, from: Dayjs, to: Dayjs, units: OpUnitType, inclusivity: string): boolean {
    var localFrom = dayjs.isDayjs(from) ? dayjs(from) : dayjs(),
        localTo = dayjs.isDayjs(to) ? dayjs(to) : dayjs();
    if (!(needle.isValid() && localFrom.isValid() && localTo.isValid())) {
        return false;
    }
    inclusivity = inclusivity || '()';
    return (
        (inclusivity[0] === '('
            ? needle.isAfter(localFrom, units)
            : !needle.isBefore(localFrom, units)) &&
        (inclusivity[1] === ')'
            ? needle.isBefore(localTo, units)
            : !needle.isAfter(localTo, units))
    );
}
